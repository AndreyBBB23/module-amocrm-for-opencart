...
$amocrm_extension_shutdown_redirect = "";

function amocrm_extension_shutdown()
{
    global $amocrm_extension_shutdown_redirect;
    if ($amocrm_extension_shutdown_redirect)
        header("location: " . $amocrm_extension_shutdown_redirect);
}

class AmoCrmController extends Controller
{

    protected $_moduleSysName = "amocrm_";
    ...
    public function __construct($registry)
    {
        ...
        if (isset($this->request->get["route"])) {
            ...
            if ($route[0] == 'extension' && $route[1] == 'extension' && isset($route[2])) {
                ...
                // Это настройки модуля, например extension/feed/amocrm_product_feed
                $this->_route = $route[1];
            } else {
    ...
    public function initButtons($data)
    {
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['save'] = $this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName(), 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
            $data['save_and_close'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&close=1', true);
        } else {
            $data['save'] = $this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName(), 'module_id=' . $this->request->get['module_id'] . '&user_token=' . $this->session->data['user_token'] . '&type=module', true);
            $data['save_and_close'] = $this->url->link('marketplace/extension', 'module_id=' . $this->request->get['module_id'] . '&user_token=' . $this->session->data['user_token'] . '&close=1', true);
        }

        $data['recheck'] = $this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName(), 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
        $data['close'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'], true);
        $data['clear'] = $this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName() . '/clear', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
        $data['download'] = $this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName() . '/downloadLogFile', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);
    ...
    protected function initBreadcrumbs($items, $data)
    {
        ...
        foreach ($newItems as $item) {
            ...
            if (isset($this->session->data['user_token']) && $isadmin) {
                $data['breadcrumbs'][] = array('href' => $this->url->link($item[0], 'user_token=' . $this->session->data['user_token'] . $params, true), 'text' => $this->language->get($item[1]), 'separator' => (count($data['breadcrumbs']) == 0 ? FALSE : ' :: '));
            } else {
    ...
    public function clear()
    {
        ...
        $this->response->redirect($this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName(), 'user_token=' . $this->session->data['user_token'] . '#tab-logs', true));
    }
    ...
    public function downloadLogFile()
    {
        ...
        if (is_file(DIR_LOGS . $this->_logFile) && file_get_contents(DIR_LOGS . $this->_logFile)) {
            ...
            $this->response->redirect($this->url->link("extension/" . $this->_route . '/' . $this->_moduleSysName(), 'user_token=' . $this->session->data['user_token'] . '&type=module', true));
        }
    }
